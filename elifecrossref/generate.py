...
class crossrefXML(object):
    ...
    def __init__(self, poa_articles, pub_date=None, add_comment=True):
        """
        set the root node
        ...
        if add_comment:
            ...
            self.comment = Comment('generated by eLife at ' + self.generated +
...
def build_crossref_xml_for_articles(poa_articles, pub_date=None, add_comment=True):
    """
    ...
    eXML = crossrefXML(poa_articles, pub_date, add_comment)
    prettyXML = eXML.prettyXML()

    # Write to file
    f = open(TMP_DIR + os.sep + eXML.elife_doi_batch_id + '.xml', "wb")
    f.write(prettyXML)
    f.close()
